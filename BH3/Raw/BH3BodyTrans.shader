	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata v)
			{
				...
				NDotL = NDotL * 0.5 + 0.5;
				o.color1 = float2(NDotL,_Opaqueness); //Color2.x其实是halfLambert
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float rg_NDotL = rgProduct + i.color1.x;
				...
				float rgFix_NDotL = rgProductFix + i.color1.x;
				...
				float3 final = diffuseColor * _Color.rgb + specColor;
